  ...
  (function(root, factory) {
    ...
    Table = (function() {
      ...
      Table.prototype.find = function(query, options, _priv) {
        var arr, exId, id, idcol, info, item, joinCols, joinInfos, joins, keyColumn, keys, name, orig_id, q, report, reqCols, res, result, ret, tblObj, value, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _ref, _ref1, _ref2;
        if (options == null) {
          options = {};
        }
        if (_priv == null) {
        ...
        query = (_priv.normalized ? query : Table._normalizeQuery(query, this._rels));
        if (query) {
        ...
        if (options.join) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            _ref1 = tblObj.find(info.query, info.options);
            for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
              result = _ref1[_k];
              orig_id = result[idcol];
              if (!joins[orig_id]) {
                joins[orig_id] = {};
              }
              if (!joins[orig_id][name]) {
                joins[orig_id][name] = [];
              }
            ...
            if ((info.offset != null) || (info.limit != null)) {
              for (id in joins) {
                ...
                arr = value[name];
                if (arr) {
                  value[name] = Table._offsetLimit(arr, info.offset, info.limit);
                }
              }
            }
            if (info.select) {
              if (typeof info.select === "string") {
                for (id in joins) {
                  value = joins[id];
                  if (value[name] != null) {
                    value[name] = value[name].map(function(v) {
          ...
          for (_l = 0, _len3 = _ref2.length; _l < _len3; _l++) {
            ...
            name = info.name;
            tblObj = this.db.table(info.tbl);
            q = Table._normalizeQuery(info.query, this._rels);
            joinCols.push(name);
            if (info.req) {
              reqCols.push(name);
            }
        ...
        for (_n = 0, _len5 = res.length; _n < _len5; _n++) {
          item = res[_n];
     ...
     */
    Table.prototype._getJoinInfos = function(joinOptions) {
      var col, joinInfo, joinInfos, k, op, options, query, tbl_col, tblname, v, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3;
      if (joinOptions === true) {
        joinOptions = {};
        _ref = this._rels;
        for (col in _ref) {
          tblname = _ref[col];
          joinOptions[col] = true;
        }
      } else if (typeof joinOptions === "string") {
        k = joinOptions;
        joinOptions = {};
        joinOptions[k] = true;
      }
      ...
      for (tbl_col in joinOptions) {
        options = joinOptions[tbl_col];
        joinInfo = this._resolveTableColumn(tbl_col, options);
        joinInfo.options = {};
        if (typeof options === "object") {
          joinInfo.name = options.as ? options.as : tbl_col;
          joinInfo.req = !options.outer;
          if (options.outer === "array") {
            joinInfo.emptyArray = true;
          }
          delete options.as;
          delete options.outer;
          delete options.explain;
          _ref1 = ["limit", "offset", "select"];
          for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
            op = _ref1[_i];
            if (options[op] != null) {
     ...
     */
    Table.prototype._resolveTableColumn = function(tbl_col, options) {
      var col, len, option, refCols, referred, subJoinInfo, tableColumn, tbl, value;
      tbl_col = tbl_col.split(".");
      len = tbl_col.length;
      (len <= 2) || err(quo(tbl_col), "is invalid expression", quo(k));
      if (len === 1) {
        if (this._rels[tbl_col[0]]) {
          col = tbl_col[0];
          tableColumn = {
            col: col + "_id",
            tbl: this._rels[col],
          ...
          if (referred) {
            ...
            (refCols.length === 1) || err("table", quo(tbl), "refers", quo(this.name), "multiply. You can specify table and column like", quo("table_name.column_name"));
            tableColumn = {
              tbl: tbl,
              col: refCols[0] + "_id",
              reltype: "N"
            ...
            (typeof options === "object" && (options.via != null)) || err("table", quo(tbl), "is not referring table", quo(this.name));
            tableColumn = this._resolveTableColumn(options.via);
            delete options.via;
            subJoinInfo = {};
    ...
    Table._normalizeQuery = function(query, rels) {
      ...
      return arrayize(query).map(function(condsList) {
        return Object.keys(condsList).reduce((function(ret, column) {
          ...
          ret[column] = arrayize(conds).map(function(cond) {
            if (cond === null) {
              return {
                equal: null
              };
    ...
    Queries.index.$in = function(col, values, list) {
      var results, value, _i, _len, _ref;
      if (!list.length) {
        return [];
      }
      results = [];
      _ref = arrayize(values);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        value = _ref[_i];
        this._idxSearchByValue(list, col, value, function(obj, data) {
          var k, keys, _j, _len1, _results;
          keys = list.keys(obj.id);
          if (keys) {
            _results = [];
            for (_j = 0, _len1 = keys.length; _j < _len1; _j++) {
              k = keys[_j];
              _results.push(results.push(list[k]));
            }
            return _results;
          }
        });
